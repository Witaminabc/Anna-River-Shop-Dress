(function ($) {
    ...
    $(document).on('change', '.yookassa_sbbol_tax_rate_select', function() {
        const infoSbbol20 = $('#if-sbbol-nds-20');
        const hasSelected20 = $('.yookassa_sbbol_tax_rate_select').filter(function() {return $(this).val() === '20';}).length > 0;

        infoSbbol20.toggle(hasSelected20);
    });

    $(document).on('change', '.yookassa_tax_rate_select', function() {
        const infoTaxRate20 = $('#if-nds-20');
        const taxRateSelects = $('.yookassa_tax_rate_select');
        const hasSelected20 = taxRateSelects.filter(function() {return $(this).val() === '4';}).length > 0;
        const hasSelected120 = taxRateSelects.filter(function() {return $(this).val() === '6';}).length > 0;

        infoTaxRate20.toggle(hasSelected20||hasSelected120);
    });
